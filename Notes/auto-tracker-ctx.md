...
|`0x0133`|`CFG_DUNGEON_ENTRANCES`|6|`0xCC`|For non mixed pools : for each dungeon entrance in the order of Deku, DC, Jabu, Forest, Fire, Water, Shadow, Spirit, BotW, Ice, GTG and Ganons castle, a null-terminated `0x16`-byte (`0x17` including the null terminator) ASCII string containing the dungeon located in it, padded with spaces on the right. For mixed pools : the string displays the hint area the dungeon is located in instead.|
|`0x01FF`|`CFG_BOSSES`|6|`0xCC`|For non mixed pools : for each dungeon entrance in the order of Deku, DC, Jabu, Forest, Fire, Water, Shadow, and Spirit, BotW, Ice, GTG and Ganons castle, a null-terminated `0x16`-byte (`0x17` including the null terminator) ASCII string containing the boss located in it, padded with spaces on the right. For mixed pools : the string displays the hint area the boss is located in instead.|
